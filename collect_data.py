...
# ------------------------------------------
# COLLECT STOCK PRICES (Yahoo Finance)
# ------------------------------------------
def collect_prices():
...
# ------------------------------------------
# COLLECT NEWS (Marketaux or Mediastack)
# ------------------------------------------
...
def collect_news_mediastack(query, start, end):
    key = os.getenv("MEDIASTACK_KEY")
    if not key:
        print("[ERROR] MEDIASTACK_KEY missing in .env file")
        return pd.DataFrame()
    ...
    while True:
        params = {
            "access_key": key,
            "languages": "en",
            "date": f"{start},{end or pd.Timestamp.utcnow().strftime('%Y-%m-%d')}",
            "keywords": query,
            "limit": 100,
            "offset": offset,
            "sort": "published_desc"
...
def collect_news():
    ...
    for t in TICKERS:
        ...
        if USE_MARKETAUX:
            # Collect news using the main symbol only (to maximize articles within API limits)
            df = collect_news_marketaux(t, START, END)
        else:
            df = collect_news_mediastack(queries[t], START, END)
            df["ticker"] = t
